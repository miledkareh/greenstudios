...
if(isset($_GET["id"])){
	...
	$action=$_GET['action'];
	if($action==1)
	$sql="select * from offerfollowup where serial=$serial";
	else if ($action==2)
	$sql="select *,(select Fullname from users where serial=offerfollowup.userid) as userN from offerfollowup where offerid=$serial";
	else if ($action==3)
