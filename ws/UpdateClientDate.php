...
include('../pages/configdb.php');
	...
	$sql="select *,(select dat from offers where customerid=customers.serial) as Dat from customers where dat ='0000-00-00 00:00:00'";
$results = mysqli_query($dbhandle,$sql)  or die(mysqli_error());
while($x = mysqli_fetch_array($results)){

	if($x['Dat']!=''){
	$sql="Update customers set dat='".$x['Dat']."' where serial=".$x['Serial'];
	$db = new DAL();		
