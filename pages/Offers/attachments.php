        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <h1 class="page-header">Offer - Attachments</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading" align="right">
							   ...
							   <button type="button" id="Add" class="btn btn-outline btn-primary" data-toggle="modal" data-target="#myModal"  >Add Attachment</button>
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Status</th>
										<th>Offer</th>
                                      <?php if($_SESSION['oce']==1){?>      <th>Edit</th> <?php }?>
...
$query = "Select *,(select trueserial from offers where serial=offerattachment.offerid) as offerserial From offerattachment where offerid=".$_GET['x']." ".$confidential." and isnew=0";
if($_SESSION['Hide']==1)
$query.=" and status='wip' ";
$query.=" order by attachementdate desc";
if($_SESSION['hidevalues']==1){$query = $query." and private=0"; }
$results = mysqli_query($dbhandle,$query)  or die(mysqli_error());
while($x = mysqli_fetch_array($results)){?>

                                    <tr class="odd gradeX" data-toggle="tooltip" data-placement="top" >                                   	
                                       <?php if ($x['isweb']==1) {?>
                                        <td><a href="../../att/<?php echo($x["status"]."/".$x["offerid"]."/".str_replace("#","%23",$x["description"]));?>" target="_blank"><?php echo($x["description"]);?></td>
                                        
                                        <?php }else{?>
                                        	 <td><a href="../../att/<?php echo($x["status"]."/".$x["offerserial"]."/".str_replace("#","%23",$x["description"]));?>" target="_blank"><?php echo($x["description"]);?></td>
                                        	 	<?php }?>
                                        	 	<td><?php echo date("d-m-Y", strtotime($x["attachementdate"]));?></td>
 <td><?php if($x['status']=='wip') echo "Work In Progress"; else if($x['status']=='regular') echo "Prices & Correspondance";?></td>
                                        <td><?php if ($x["Private"]!=1) {echo("No");} else {echo("Yes");}?></td>
    ...
    <div class="col-lg-6 nopadding" align="center" <?php if($_SESSION['Hide']==1){?>style="display: none;" <?php }?>>
    	<label>Prices & Correspondance</label>
          ...
          <input id="images1" name="images[]" type="file" multiple>
   	  	   ...
   	  	   <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="private" value="private">Offer
                                    </label>
                               </div>
  <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="main" value="main">Main Image
                                    </label>
                               </div>
                                  <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="confidential" value="confidential" >confidential  &nbsp;
                                    </label>
                               </div>
