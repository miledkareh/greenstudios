...
$proj="";
  ...
  while($row = mysqli_fetch_array($result)){
   $proj.=$row['ProjectName']." / ".$row['Description'].",".$row['SERIAL']."!";
...
and maintenancedetails.UserId='".$User."' order by checkin.dat DESC ");
   ...
   while($row = mysqli_fetch_array($result)){
   $proj.=$row['ProjectName'].",".$row['Serial']."/".$row['dat']."!".$row['SERIAL']."AZX!";
...
$pwork="";
   $result = mysqli_query($con,"select * from   pwork where 1");
  while($row = mysqli_fetch_array($result)){
...
$pwork="";
   $result = mysqli_query($con,"select Distinct (trade) from pesticide ");
  while($row = mysqli_fetch_array($result)){
   $pwork.=$row['trade']."U,U";
