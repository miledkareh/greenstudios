...
$zonesnamesend=$_POST['zonesnamesend'];
 ...
 $mainID=$arr[0];
  ...
  $sql="UPDATE checkin SET  update1=1,checkout=1,checkoutdate='".$gooddat."'  where visit=$ID";
      $result = mysqli_query($con,$sql);	
...
$z=explode("K,K",$zonesnamesend);
 ...
 $sql="INSERT INTO `irrigationtime`( `dat`, `zone`, `starttime`, `endtime`, `maintenanceid`,maintenancedetail_id) VALUES ('".date("y-m-d")."','".$z[$i]."','".$stime[$i]."','".$etime[$i]."','".$mainID."','".$ID."')";
 	...
 	//try{
$sql="INSERT INTO `readingm`(  `dat`, `ecclean`, `ecfertilized`, `plantnumber`, `ph`, `maintenanceid`, `injector`,fertelizedmix,alarm,maintenancedetail_id) VALUES 
('".date("y-m-d")."','".$water."','".$mix."','".$nbofplants."','".$ph."','".$mainID."','".$inj."','".$fert."','".$alarm."','".$ID."')";
 $result = mysqli_query($con,$sql);
