...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------',270);
	include('../configdb.php');
	 $query = "Select * from genpar where country in (select country from offers where serial in (select offerid from maintenances where serial in (select maintenanceid from maintenancedetails where serial in (select visit from checkin where serial=".$_GET['x'].")))) limit 1";
		//echo($_GET["to"]);
  ...
  $this->SetFont('zapfdingbats','',25);
	...
	$this->SetTextColor(0,0,0);
	  $this->Cell(5,5,"",0,'L',0);
		...
		$this->Cell(30,5,"",0,'L',0);
		$this->SetTextColor(69,68,68);
		$this->MultiCell(55,5,$x['gsnotes'],0,'L',0);
			$this->Ln(5);
		}
