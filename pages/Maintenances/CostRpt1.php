...
employees as Employees,
 (select fname from customers where serial in (select customerid from offers where serial in (select offerid from maintenances where serial =".$_GET['x']."))) as fname,
  ...
  (select company from customers where serial in (select customerid from offers where serial in (select offerid from maintenances where serial =".$_GET['x']."))) as company,

  (select telephone from customers where serial in (select customerid from offers where serial in (select offerid from maintenances where serial =".$_GET['x']."))) as phone,

(select email from customers where serial in (select customerid from offers where serial in (select offerid from maintenances where serial =".$_GET['x']."))) as email,

(select notes from customers where serial in (select customerid from offers where serial in (select offerid from maintenances where serial =".$_GET['x']."))) as notes,
...
where accepted=1 and maintenanceid=".$_GET['x'];
	...
	//	echo "ERROR";
		...
		$yy=$this->GetY();
		$this->SetXY(40, $yy);
		$this->MultiCell(67,4,$x['company'],0,'L',0);
		$this->SetXY(15, $yy);
	
		
		$this->Cell(25,4,'ATTENTION',0,0,'L');
		$this->SetTextColor(0,0,0);
		$this->SetXY(110, $yy);
		$this->MultiCell(20,4,'PROJECT',0,'L',0);
		$this->SetTextColor(69,68,68);
		$this->SetXY(136, $yy);
		$this->MultiCell(48,4,$x['projectN'],0,'L',0);
		$this->SetTextColor(0,0,0);
		$yy+=10;
		
		$this->SetXY(15, $yy);
		$this->Cell(25,4,'CONTACT',0,0,'L');
		$this->SetTextColor(0,0,0);
		$this->SetXY(40, $yy);
		$this->Cell(25,4,"asdsadsad".$x['fname']." ".$x['lname'],0,0,'L');
		$yy+=5;
		$this->SetXY(40, $yy);
		$this->Cell(25,4,$x['email'],0,0,'L');
		$yy+=5;
		$this->SetXY(40, $yy);
		$this->Cell(25,4,$x['phone'],0,0,'L');
		$yy-=16;
		$this->SetXY(110, $yy);
	 ...
	 while($y = mysqli_fetch_array($result)){
		...
		$this->Ln();
		$query = "Select *,(select description from servicescost where serial=visitcost.serviceid) as description   from visitcost where visitid =".$x['Serial'];
