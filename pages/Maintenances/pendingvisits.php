        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <h1 class="page-header"><?php echo $_GET['k'];?> - Pending Visits</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
...
$query = "select *,maintenancedetails.serial as SERIAL,(select sent from checkin where visit=maintenancedetails.Serial)as sent1,
(select notes from checkin where visit=maintenancedetails.Serial)as Notes,
...
(select checkin from checkin where visit=maintenancedetails.Serial)as checkin,
(select FullName from users where serial = maintenancedetails.userid ) as User, 
(select count(serial) as cnt from maintenanceattachement where maintenancedetailid =maintenancedetails.serial and approved=0) as Attachs,
Employees From maintenancedetails,checkin where checkin.visit=maintenancedetails.serial   and  maintenanceid=".$_GET['x']." 
and maintenancedetails.accepted=0  

order by (select checkoutdate from checkin where visit=maintenancedetails.Serial) desc";
...
while($x = mysqli_fetch_array($results)){?>
                                    ...
                                    <tr class="odd gradeX" >              
										...
										{?>
                                        	...
                                        	<?php }}
                                        ?></td>
                                        <td><a href="readingspending.php?x=<?php echo $x['SERIAL'];  ?>&y=<?php echo($_GET["k"]);?>">Readings </a></td>
                                      <?php if($_SESSION['mce']==1){?>
                                      	<?php if($x['checkout']==1){?>
                                      		
<td class="center">
<p class="fa fa-flag-o"></p> <a  href="#" id="Edit_<?php echo($x["SERIAL"]);?>,<?php echo($x["checkinid"]);?>"  data-toggle="modal" data-target="#myModal2" >Check Out &nbsp;   </a>
</td>

<?php } else { ?>
<td class="center">
<a  id="Checkout_<?php echo($x["SERIAL"]);?>,<?php echo($x["checkinid"]);?>"  data-toggle="modal" data-target="#myModal2" ><p class="fa fa-flag-o"></p> Check Out</a>
</td>
<?php }?>
<td class="center">
<a  id="Edit_<?php echo($x["SERIAL"]);?>,<?php echo($x["checkinid"]);?>"  data-toggle="modal" data-target="#myModal" ><p class="fa fa-edit"></p> Edit</a>
</td>
<?php }  ?>
...
<?php }  ?>
 <td><a target="_blank" href="./Transaction.php?x=<?php echo($x['checkinid']);?>">Print</a></td>
...
$results = mysqli_query($dbhandle,$query)  or die(mysqli_error());
 ...
 <?php 
	...
	<div class="checkbox">
        ...
        <div class="checkbox">
        <label>
        <input type="checkbox" id="approved" value="1">approved
        </label>
        </div>
	<label>Rating</label>
	<div>
    ...
    <label id="sentalert" style="color: red" >Report should be sent!</label>
        </div>
        <div class="modal-footer" >
...
<script src="../../js/visitspending.js"></script>
</body>
