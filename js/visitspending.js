	 ...
	 $('#sent').change(function() {
        if(this.checked) {
          document.getElementById('add2').disabled=false;
             document.getElementById('sentalert').hidden=true;
        }
        else{
        	 document.getElementById('add2').disabled=true;
             document.getElementById('sentalert').hidden=false;
        }
	...
	if(count>0)
	{
		if( serial==0)
			...
			{		
				...
				if(data[0]["client"]==''){
					document.getElementById("client").value=findGetParameter('k');
				}
				else
...
switch(type){
	case 'jpg': initialPreviewConfig.push({caption: data[i]["description"], width: "120px",  url: "../../ws/ws_Moveimage.php?id=" + data[i]["serial"] + "&tablename=visitattachment"   , key: data[i]["serial"],downloadUrl: "../../att/visits/"+data[i]["description"]+"/"+data[i]["description"] });
				break;
	case 'png': initialPreviewConfig.push({caption: data[i]["description"], width: "120px",  url: "../../ws/ws_Moveimage.php?id=" + data[i]["serial"] + "&tablename=visitattachment"   , key: data[i]["serial"],downloadUrl: "../../att/visits/"+data[i]["description"]+"/"+data[i]["description"]});
				break;
	case 'gif': initialPreviewConfig.push({caption: data[i]["description"], width: "120px",  url: "../../ws/ws_Moveimage.php?id=" + data[i]["serial"] + "&tablename=visitattachment"   , key: data[i]["serial"],downloadUrl: "../../att/visits/"+data[i]["description"]+"/"+data[i]["description"]});
				break;
	case 'jpeg': initialPreviewConfig.push({caption: data[i]["description"], width: "120px", url: "../../ws/ws_Moveimage.php?id=" + data[i]["serial"] + "&tablename=visitattachment"   , key: data[i]["serial"],downloadUrl: "../../att/visits/"+data[i]["description"]+"/"+data[i]["description"]});
				break;
	case 'pdf': initialPreviewConfig.push(  {type: "pdf", caption: data[i]["description"],   url: "../../ws/ws_Moveimage.php?id=" + data[i]["serial"] + "&tablename=visitattachment"   , key: data[i]["serial"],downloadUrl: "../../att/visits/"+data[i]["description"]+"/"+data[i]["description"]});
				break;
		...
		subject=$('#subject').val();
			...
			document.getElementById("elblalert").style.visibility='visible';
			 ...
			 $.ajax({
			  ...
			  data: ({id:ID1,gsnote:gsnote,subject:subject,description:description,email:email}),
...
if(ID==0)	
	...
	//		alert(sent+' '+gsnotes+' '+Latitude+' '+Longitude+' '+notes+' '+today+' '+rate+' '+client+' '+checkoutdate+' '+dataa);
		...
		UpdateAttachment1(ID,sent,gsnotes,Latitude,Longitude,notes,today,rate,client,checkoutdate,dataa,approved);
...
//////////////////////////////////////////////////////////////////////////////////////////////////
function UpdateAttachment1(serial,sent,gsnotes,Latitude,Longitude,notes,today,rate,client,checkoutdate,dataa,approved)
	{
	 	 
		  $.ajax({
			  type: 'GET',
			  url: "../../ws/ws_tcheckin.php",
			  data: ({action:4,serial :serial,approved:approved,sent:sent,gsnotes:gsnotes,dataa:dataa,checkoutdate:checkoutdate,Latitude:Latitude,Longitude:Longitude,notes:notes,today:today,rate:rate,client:client}),
