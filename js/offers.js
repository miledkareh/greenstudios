...
$('#date').change(function() {
    dat=$('#date').val();
    dat=new Date(dat);
    dat1=dat.setDate(dat.getDate()+7);
 	
    $('#duedate').val(formatDate(dat1));
    dat=$('#date').val();
    dat=new Date(dat);
 	dat1=dat.setMonth(dat.getMonth()+3);
    $('#kdate').val(formatDate(dat1));
  

});
$('#cost').change(function() {
    cost=$('#cost').val();
   document.getElementById('gross').value=((Number($('#offervalue').val())-cost)/$('#offervalue').val()*100).toFixed(2);
...
if(zoness==0)	
				{
					...
					//window.open("../pages/upload.php?x="+ document.getElementById("attachment").value,"_blank","width=0,height=0");
					addzoness(dato,ID,descriptiona);
		...
		Updatezoness(zoness,dato,ID,descriptiona);
	...
	$("#zoness").empty();
		...
		{			
		   ...
		   $.each(data, function(index, row) {
				item = "<tr id='tr_"+row.serial+"'>";
                item+=   "<td>"+row.dat+"</td>";
                item+="<td><a  id='Edittt_"+row.serial+"'  data-toggle='modal' data-target='#myModala' ><p class='fa fa-edit'></p> "+row.description+"</a></td>";
...
$("#elegationlines").on("change paste keyup", function() {
    
elegationlines=$("#elegationlines").val();
   
			 width=$("#width").val();
			 
  document.getElementById("emittersqte").value=5*width*elegationlines; 
...
$('input[type=radio][name=regularity]').change(function() {
...
$("#height").on("change paste keyup", function() {
  ...
  testt=$('input:radio[name=regularity]:checked').val();
 
  if(testt==0){ height=$("#height").val();	
			 width=$("#width").val();
			 
			 document.getElementById("cm").value=Math.round(height*width);}

if(testt==1){ height=$("#height").val();	
			 width=$("#width").val();
			 
			 document.getElementById("cm").value=Math.round(((2*height)+Number(width))/2);}
...
$("#doors").change(function() {
    if(this.checked) {
         document.getElementById("doorsqte").disabled=false;
         document.getElementById("doorwid").disabled=false;
         document.getElementById("doorhei").disabled=false;
    }
    else{ document.getElementById("doorsqte").disabled=true;
			document.getElementById("doorwid").disabled=true;
         document.getElementById("doorhei").disabled=true;}
});
...
$("#windows").change(function() {
    if(this.checked) {
         document.getElementById("windowsqte").disabled=false;
         document.getElementById("windowwid").disabled=false;
         document.getElementById("windowhei").disabled=false;
    }
    else{ document.getElementById("windowsqte").disabled=true;
document.getElementById("windowwid").disabled=true;
         document.getElementById("windowhei").disabled=true;}
});

$("#interial").change(function() {
    if(this.checked) {
         document.getElementById("interialqte").disabled=false;
    }
    else{ document.getElementById("interialqte").disabled=true;}
});

$("#exterial").change(function() {
    if(this.checked) {
         document.getElementById("exterialqte").disabled=false;
    }
    else{ document.getElementById("exterialqte").disabled=true;}
});
	...
	$("#convert").click(function(){
		$.ajax({
			type: 'GET',
			url: "../../ws/ws_convert.php",
			data: ({currency:$('#currency1').val(),dat:$('#date').val()}),
			dataType: 'json',
			timeout: 5000,
			success: function(data, textStatus, xhr) 
			{
					  ...
					  data = JSON.parse(xhr.responseText);	
					if($('#currency1').val()==2)
					document.getElementById('offervalue').value=  $('#valuecurrency').val();
					else
					document.getElementById('offervalue').value= $('#valuecurrency').val()*data[0]['ToAmount'] / data[0]['FromAmount'];
...
doorhei=0;
 	...
 	var rad=$('input:radio[name=regularity]:checked').val();


if ($("#doors").is(':checked')) {  doorsqte=$("#doorsqte").val();}
if ($("#windows").is(':checked')) {  windowsqte=$("#windowsqte").val();}
if ($("#interial").is(':checked')) {  interialqte=$("#interialqte").val();}
if ($("#exterial").is(':checked')) {  exterialqte=$("#exterialqte").val();}

jamb=0+$("#jamb").val();
height=0+$("#height").val();	
width=0+$("#width").val();
buildupp=$("#buildupp").val();	
 
stationnb=$("#stationnb").val();	
caliber=$("#caliber").val();
emittertype= $("#emittertype").val();
elegationlines=$("#elegationlines").val();	
emittersqte=$("#emittersqte").val();
flowsensors= $("#flowsensors").val();



windowwid= $("#windowwid").val();
windowhei=$("#windowhei").val();	
doorwid=$("#doorwid").val();
doorhei= $("#doorhei").val();

 
subcontractor= $("#subcontractor").val();
 if(subcontractor=='')subcontractor=-1;
...
if(document.getElementById('specified').checked==true)
	specified = 1;
	else specified=0;
if(document.getElementById('hprobability').checked==true)
	hp = 1;
...
valuecurrency= $("#valuecurrency").val();
...
if(ID==0)	
				{
					...
					//window.open("../pages/upload.php?x="+ document.getElementById("attachment").value,"_blank","width=0,height=0");
 addAttachment(valuecurrency,subcontractor,specified,cost,gross,kickoffna,maincontractor,maincontractorreferral,currency,projectname,date,duedate,country,city,pe,client,clien,consultant,architect,larchitect,contractor,refferal,refferaln,gw,intt,ext,areaa,rg,rgarea,ls,lsarea,buildup,tsupport,tstartdate,tenddate,design,dstartdate,denddate,kdate,offerref,offervalue,offerremaining,cancelreason,notes,today,dealer,bd,troom,status,statusdate,hp,attachment,hpdate,bddate,agentdate,attachment1,image1,image2,image3,image4,image5,image6,image7,image8,image9,image10,image11,image12,rad,windowsqte,doorsqte,width,height,buildupp,interialqte,exterialqte,stationnb,caliber,emittertype,elegationlines,emittersqte,flowsensors,toto,jamb,windowwid,windowhei,doorwid,doorhei);
					
					
				}
		else{	
		//	window.open("../pages/upload.php?x="+ attachment,"_blank","width=0,height=0");
		UpdateAttachment(valuecurrency,subcontractor,specified,ID,cost,gross,kickoffna,maincontractor,maincontractorreferral,currency,projectname,date,duedate,country,city,pe,client,clien,consultant,architect,larchitect,contractor,refferal,refferaln,gw,intt,ext,areaa,rg,rgarea,ls,lsarea,buildup,tsupport,tstartdate,tenddate,design,dstartdate,denddate,kdate,offerref,offervalue,offerremaining,cancelreason,notes,today,dealer,bd,troom,status,statusdate,hp,hpdate,bddate,agentdate,image1,image2,image3,image4,image5,image6,image7,image8,image9,image10,image11,image12,rad,windowsqte,doorsqte,width,height,buildupp,interialqte,exterialqte,stationnb,caliber,emittertype,elegationlines,emittersqte,flowsensors,toto,jamb,windowwid,windowhei,doorwid,doorhei);
...
function UpdateAttachment(valuecurrency,subcontractor,specified,serial,cost,gross,kickoffna,maincontractor,maincontractorreferral,currency,projectname,date,duedate,country,city,pe,client,clien,consultant,architect,larchitect,contractor,refferal,refferaln,gw,intt,ext,areaa,rg,rgarea,ls,lsarea,buildup,tsupport,tstartdate,tenddate,design,dstartdate,denddate,kdate,offerref,offervalue,offerremaining,cancelreason,notes,today,dealer,bd,troom,status,statusdate,hp,hpdate,bddate,agentdate,image1,image2,image3,image4,image5,image6,image7,image8,image9,image10,image11,image12,rad,windowsqte,doorsqte,width,height,buildupp,interialqte,exterialqte,stationnb,caliber,emittertype,elegationlines,emittersqte,flowsensors,toto,jamb,windowwid,windowhei,doorwid,doorhei)
	{
	//	alert(pe);
//alert("hi");
...
//alert(image1+" "+image2+" "+image3+" "+image4+" "+image5+" "+image6+" "+image7+" "+image8+" "+image9+" "+image10+" "+image11+" "+image12);	
		  ...
		  $.ajax({
			  type: 'GET',
			  url: "../../ws/ws_toffers.php",
			  data: ({action:3,serial :serial,valuecurrency:valuecurrency,subcontractor:subcontractor,specified:specified,cost:cost,gross:gross,kickoffna:kickoffna,maincontractor:maincontractor,maincontractorreferral:maincontractorreferral,currency:currency,projectname:projectname,date:date,duedate:duedate,country:country,city:city,pe:pe,client:client,clien:clien,consultant:consultant,architect:architect,larchitect:larchitect,contractor:contractor,refferal:refferal,refferaln:refferaln,gw:gw,intt:intt,ext:ext,areaa:areaa,rg:rg,rgarea:rgarea,ls:ls,lsarea:lsarea,buildup:buildup,tsupport:tsupport,tstartdate:tstartdate,tenddate:tenddate,design:design,dstartdate:dstartdate,denddate:denddate,kdate:kdate,offerref:offerref,offervalue:offervalue,offerremaining:offerremaining,cancelreason:cancelreason,notes:notes,today:today,dealer:dealer,bd:bd,troom:troom,status:status,statusdate:statusdate,hprobability:hp,hpdate:hpdate,bddate:bddate,agentdate:agentdate,image1:image1,image2:image2,image3:image3,image4:image4,image5:image5,image6:image6,image7:image7,image8:image8,image9:image9,image10:image10,image11:image11,image12:image12,rad:rad,windowsqte:windowsqte,doorsqte:doorsqte,width:width,height:height,buildupp:buildupp,interialqte:interialqte,exterialqte:exterialqte,stationnb:stationnb,caliber:caliber,emittertype:emittertype,elegationlines:elegationlines,emittersqte:emittersqte,flowsensors:flowsensors,toto:toto,jamb:jamb,windowwid:windowwid,windowhei:windowhei,doorwid:doorwid,doorhei:doorhei}),
...
//=====================================================================================
function addAttachment(valuecurrency,subcontractor,specified,cost,gross,kickoffna,maincontractor,maincontractorreferral,currency,projectname,date,duedate,country,city,pe,client,clien,consultant,architect,larchitect,contractor,refferal,refferaln,gw,intt,ext,areaa,rg,rgarea,ls,lsarea,buildup,tsupport,tstartdate,tenddate,design,dstartdate,denddate,kdate,offerref,offervalue,offerremaining,cancelreason,notes,today,dealer,bd,troom,status,statusdate,hp,attachment,hpdate,bddate,agentdate,attachment1,image1,image2,image3,image4,image5,image6,image7,image8,image9,image10,image11,image12,rad,windowsqte,doorsqte,width,height,buildupp,interialqte,exterialqte,stationnb,caliber,emittertype,elegationlines,emittersqte,flowsensors,toto,jamb,windowwid,windowhei,doorwid,doorhei)
	{		
	//	alert("hi");
...
//	alert(attachment+" "+paperid+" "+notes);
		  ...
		  $.ajax({
			  ...
			  type: 'GET',
			  url: "../../ws/ws_toffers.php",
			  data: ({action:1,valuecurrency:valuecurrency,cost:cost,gross:gross,subcontractor:subcontractor,specified:specified,kickoffna:kickoffna,maincontractor:maincontractor,maincontractorreferral:maincontractorreferral,currency:currency,projectname:projectname,date:date,duedate:duedate,country:country,city:city,pe:pe,client:client,clien:clien,consultant:consultant,architect:architect,larchitect:larchitect,contractor:contractor,refferal:refferal,refferaln:refferaln,gw:gw,intt:intt,ext:ext,areaa:areaa,rg:rg,rgarea:rgarea,ls:ls,lsarea:lsarea,buildup:buildup,tsupport:tsupport,tstartdate:tstartdate,tenddate:tenddate,design:design,dstartdate:dstartdate,denddate:denddate,kdate:kdate,offerref:offerref,offervalue:offervalue,offerremaining:offerremaining,cancelreason:cancelreason,notes:notes,today:today,dealer:dealer,bd:bd,troom:troom,status:status,statusdate:statusdate,hprobability:hp,hpdate:hpdate,bddate:bddate,agentdate:agentdate,image1:image1,image2:image2,image3:image3,image4:image4,image5:image5,image6:image6,image7:image7,image8:image8,image9:image9,image10:image10,image11:image11,image12:image12,rad:rad,windowsqte:windowsqte,doorsqte:doorsqte,width:width,height:height,buildupp:buildupp,interialqte:interialqte,exterialqte:exterialqte,stationnb:stationnb,caliber:caliber,emittertype:emittertype,elegationlines:elegationlines,emittersqte:emittersqte,flowsensors:flowsensors,toto:toto,jamb:jamb,windowwid:windowwid,windowhei:windowhei,doorwid:doorwid,doorhei:doorhei}),
			  dataType: 'json',
			  timeout: 15000,
			  success: function(data, textStatus, xhr) 
			  {
...
document.getElementById("cm").value=data[0]["height"]*data[0]["width"];
document.getElementById("buildupp").value=data[0]["buildupp"];
document.getElementById("toto").value=data[0]["toto"];
document.getElementById("jamb").value=data[0]["jamb"];
...
document.getElementById("windowwid").value=data[0]["windowwid"] ;
document.getElementById("windowhei").value=data[0]["windowhei"];
document.getElementById("doorwid").value=data[0]["doorwid"];
document.getElementById("doorhei").value=data[0]["doorhei"]


	 var values=data[0]["subcontractor"];
			 // $.each(values.split(","), function(i,e){
			 	 
	   //   $("#subcontractor option[value='" + e + "']").prop("selected", true);
	   //  });

 values=values.split(',');
$("#subcontractor").val(values).trigger("change");



document.getElementById("stationnb").value=data[0]["stationnb"];
document.getElementById("caliber").value=data[0]["caliber"];
document.getElementById("emittertype").value=data[0]["emittertype"];
document.getElementById("elegationlines").value=data[0]["elegationlines"];
document.getElementById("flowsensors").value=data[0]["flowsensors"];
document.getElementById("emittersqte").value=data[0]["emittersqte"];
...
if(	data[0]["rad"]==1)
		...
		if(	data[0]["doorsqte"]==0)
			document.getElementById("doors").checked = false;
			else{
			document.getElementById("doors").checked = true;
			document.getElementById("doorsqte").value=data[0]["doorsqte"];}



if(	data[0]["interialqte"]==0)
			document.getElementById("interial").checked = false;
			else{
			document.getElementById("interial").checked = true;
			document.getElementById("interialqte").value=data[0]["interialqte"];}


if(	data[0]["exterialqte"]==0)
			document.getElementById("exterial").checked = false;
			else{
			document.getElementById("exterial").checked = true;
			document.getElementById("exterialqte").value=data[0]["interialqte"];}





	if(	data[0]["windowsqte"]==0)
			document.getElementById("windows").checked = false;
			else{
			document.getElementById("windows").checked = true;
document.getElementById("windowsqte").value=data[0]["windowsqte"];
}
...
document.getElementById("gross").value=data[0]["gross"];
...
if(	data[0]["specified"]==1)
			document.getElementById("specified").checked = true;
			else
			document.getElementById("specified").checked = false;
...
document.getElementById("valuecurrency").value=data[0]["valuecurrency"];
