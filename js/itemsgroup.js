...
if(ID==0)	
				{
					...
					addAttachment(specialty,color,ordernb);
					
					
				}
		else{ 
		//	window.open("../pages/upload.php?x="+ attachment,"_blank","width=0,height=0");
		UpdateAttachment(ID,specialty,color,ordernb);
...
function UpdateAttachment(serial,specialty,color,ordernb)
	{
		
		  $.ajax({
			  type: 'GET',
			  url: "../../ws/ws_titemsgroup.php",
			  data: ({action:3,serial :serial,specialty:specialty,color:color,ordernb:ordernb}),
...
//=====================================================================================
function addAttachment(specialty,color,ordernb)
	{		
...
//	alert(attachment+" "+paperid+" "+notes);
		  $.ajax({
			  ...
			  type: 'GET',
			  url: "../../ws/ws_titemsgroup.php",
			  data: ({action:1,specialty:specialty,color:color,ordernb:ordernb}),
...
////////////////////////////////////////////////////////////////////////////////////////	
	...
	if(count>0)
	{
		if( serial==0)
			...
			{		
			document.getElementById("specialty").value=data[0]["description"];
		document.getElementById("color").value=data[0]["color"];
		document.getElementById("ordernb").value=data[0]["order2"];
