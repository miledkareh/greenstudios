...
dat=$('#date').val();
		...
		UpdateAttachment(ID,offerid,dat);
			//location.reload();
...
function UpdateAttachment(serial,offerid,today)
	{
		  ...
		  $.ajax({
			  type: 'GET',
			  url: "../../ws/ws_tmattachments.php",
			  data: ({action:7,serial:serial,today:today,offerid:offerid}),
...
switch(type){
	...
	case 'jpeg': initialPreviewConfig.push({caption: data[i]["description"], width: "120px", url: "../../ws/ws_deleteimage.php?id=" + data[i]["Serial"] + "&tablename=maintenanceattachement"   , key: data[i]["Serial"],downloadUrl: "../../att/wip/"+data[i]["description"] });
				break;
	case 'pdf': initialPreviewConfig.push(  {type: "pdf", caption: data[i]["description"], url: "../../ws/ws_deleteimage.php?id=" + data[i]["serial"] + "&tablename=offerattachment"   , key: data[i]["serial"],downloadUrl: "../../att/wip/"+data[i]["description"]});
				break;
